...
class ExcelSheetMaster:
    ...
    def is_in_range(self, row_index, column_index, min_border, max_border, include_min: bool, include_max: bool):
        ...
        :return: bool
        """
        value = self.sheet.cell_value(row_index, column_index)
        if include_min:
            if include_max:
    ...
    def is_all_columns_elements_in_range(self, column_info_list: list):
        """
        ...
        """
        res_array = np.zeros(shape=(self.sheet.nrows, self.sheet.ncolumns), dtype=int)
